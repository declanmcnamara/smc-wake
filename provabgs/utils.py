...
def transform_thetas(thetas, **kwargs):
    ...
    for j in range(len(sizes)):
        if j == 0:
            ...
            new[:, spot : new_spot + 1] = my_t_priors[j].transform(
                thetas[:, spot:new_spot]
            )
            spot = new_spot + 1
        else:
            new_spot = sizes[j] + spot
            new[:, spot:new_spot] = my_t_priors[j].transform(
                thetas[:, spot - 1 : new_spot - 1]
            )
            spot = new_spot
...
def gather_weights(**kwargs):
    ...
    smooth = kwargs["smooth"]
    print(
        "Gathering SMC from noise level {}, mult noise {}, smoothing={}".format(
            noise, multiplicative_noise, smooth
        )
    )
    ...
    for j in range(n_pts):
        weights = torch.load(
            "./exp3/tmp/weights_noise={},mult={},smooth={},smoothparam={}_{}.pt".format(
                noise, multiplicative_noise, smooth, smooth_param, j
            )
        )
        log_weights = torch.load(
            "./exp3/tmp/log_weights_noise={},mult={},smooth={},smoothparam={}_{}.pt".format(
                noise, multiplicative_noise, smooth, smooth_param, j
            )
        )[:, -1]
        items = torch.load(
            "./exp3/tmp/items_noise={},mult={},smooth={},smoothparam={}_{}.pt".format(
                noise, multiplicative_noise, smooth, smooth_param, j
            )
        )[:, -1, ...]
...
def construct_smc_samplers(data, **kwargs):
    ...
    for j in range(len(data)):
        ...
        init_weights = (nn.Softmax(0)(init_log_weights)).view(-1)
        final_target_fcn = (
            lambda z: log_t_prior(z, **kwargs) + log_target(z, sed, **kwargs).cpu()
        )
